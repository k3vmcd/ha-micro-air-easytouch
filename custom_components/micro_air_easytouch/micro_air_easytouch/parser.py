        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(retries):
                try:
                    ...
                    if result:
                        _LOGGER.debug(
                            "Authentication successful on attempt %d/%d",
                            attempt + 1,
                            retries,
                        )
                        return True
                    _LOGGER.debug(
                        "Authentication returned False on attempt %d/%d",
                        attempt + 1,
                        retries,
                    )
                    ...
                    last_exception = e
                    _LOGGER.debug(
                        "Authentication attempt %d/%d failed: %s",
                        attempt + 1,
                        retries,
                        str(e),
                    )
...
class MicroAirEasyTouchBluetoothDeviceData(BluetoothData):
    ...
    def __init__(
        self, password: str | None = None, email: str | None = None
    ) -> None:
    ...
    async def _write_gatt_with_retry(
        ...
        for attempt in range(retries):
            try:
                if not self._client or not self._client.is_connected:
                    if not await self._reconnect_and_authenticate(
                        hass, ble_device
                    ):
                        return False
    ...
    async def _reconnect_and_authenticate(
    ...
    async def send_command(
        ...
        try:
            ...
            return await self._write_gatt_with_retry(
